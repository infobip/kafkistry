...
<#-- @ftlvariable name="unwantedClusterQuotas" type="java.util.Map<java.lang.String, com.infobip.kafkistry.service.quotas.QuotasInspection>" -->
...
<head>
    ...
    <title>Kafkistry: Delete unwanted entity quotas</title>
</head>
...
<div class="container">
    <h2><#include "../../common/backBtn.ftl"> Bulk delete unwanted entity quotas on clusters</h2>
    <hr/>

    <h4>Going to delete Entity quotas on clusters</h4>
    <br/>
    <p>
    ...
    <#if unwantedClusterQuotas?size gt 0>
        <button id="bulk-delete-entity-quotas-btn" class="btn btn-danger btn-sm" data-entity-id="${entity.asID()}">
            Unwanted quotas on all clusters (${unwantedClusterQuotas?size})
        </button>
    <#else>
        <p><i>No clusters need deletion of entity quotas</i></p>
    </#if>
