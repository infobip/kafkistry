...
<#-- @ftlvariable name="unwantedBrokerId" type="java.lang.Integer" -->
...
<script>
    function getThrottleBytesPerSec() {
        //we are not moving data, only re-ordering preferred leader replicas
        return 0;
    }
</script>
...
<div class="container">
    <h1>Topic re-assign unwanted broker not to be leader</h1>
    <hr>
    <br>
    <p>Broker being unwanted leader: <strong>${unwantedBrokerId?c}</strong></p>
    ...
    <button id="apply-re-assignments-btn" class="btn btn-warning btn-sm" data-topic-name="${topicName}"
        ...
        Apply unwanted leader re-assignments
    </button>
